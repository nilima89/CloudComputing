...
public class PageRankReduce extends Reducer<LongWritable, Text, LongWritable, Text>{
	...
	public void reduce(LongWritable key, Iterable<Text> values,
		...
		for (Text value: values){
			...
			// summation of pagerank of <key, vale> = <URL, pagerank of this URL>
			if(!value.toString().contains("#")){
				sumOfRankValues += Double.parseDouble(value.toString());
			}
			else{
				targetUrlsList = new String(value.toString());
			}
			
		} // end for loop
		
		sumOfRankValues = (double) 0.85*sumOfRankValues+0.15*(1.0)/(double)numUrls;
